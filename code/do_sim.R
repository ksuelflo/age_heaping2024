...
for (i in seq_along(all_folders)){
  ...
  #Creating folders to put models/summaries in at the end for each ith sim setting
  ...
  for (j in seq_along(files)){
    ...
    saveRDS(list_res, str_c(folder_model, "/", str_replace(all_files[j], "sim", "res")))
    
    #Save the summary results somewhere (Probably new folder) STILL NEED RELATIVE PATH
    saveRDS(summary_results, str_c(folder_summary, "/", str_replace(all_files[j], "sim", "summary")))
  }
}
